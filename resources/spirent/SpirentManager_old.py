...
class SpirentManager(SpirentClient):
    """ Önce update_test_server_session_or_exit metoduyla test sunucusunu doğru Çekirdek Şebekeye yönlendirelim.
    Sonra testi koşturalım.
    """
        ...
        if test_params and ts_params:
            ...
            try:
                ...
                update_content = template.render(
                    ...
                    n6_ip=upf_ip_in,
                    ue_id=h_mcc_in + h_mnc_in + test_params['msin'],
...
# ---------------------------------------------------------------------------------
    ...
    def update_test_server_session_or_exit(self, amf_ip, spirent_ts_name test_id, lib_id, h_mnc, h_mcc, test_duration, spirent_ts_id, spirent_gnb_ip, perm_key, op_key, spirent_dn_ip, upf_ip, msin, spirent_dn_interface, spirent_gnb_interface):
